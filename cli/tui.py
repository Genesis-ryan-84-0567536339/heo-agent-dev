...
GEMINI_DIR = os.environ.get("GEMINI_DIR", os.path.join(AUTH_DIR, "home", ".gemini"))
...
def step_agy_auth(agy_bin, config):
    ...
    # 2. Quy trình xác thực trực tiếp trên Terminal TTY
    while True:
        console.print(Panel(
            "[bold yellow]⚡ BƯỚC 1: XÁC THỰC TÀI KHOẢN GOOGLE CHO AGY CLI[/bold yellow]\n\n"
            "AGY CLI sẽ tạo đường link đăng nhập Google OAuth ngay bên dưới.\n\n"
            "📌 [bold cyan]HƯỚNG DẪN 3 BƯỚC THỰC HIỆN:[/bold cyan]\n"
            "  1. Khi link [bold green]https://accounts.google.com/o/oauth2/auth?...[/bold green] xuất hiện,\n"
            "     hãy [bold underline]click hoặc copy link[/bold underline] đó dán vào trình duyệt Web (Chrome, Edge...).\n"
            "  2. Đăng nhập Google và nhấn [bold green]Cho phép (Allow)[/bold green].\n"
            "  3. Copy mã [bold cyan]Authorization Code[/bold cyan] từ trình duyệt, quay lại đây dán vào rồi ấn [bold]Enter[/bold].\n"
            "  [dim](Lưu ý: Thời gian chờ nhập mã là 60 giây)[/dim]",
...
def run_services_dashboard(agy_bin, config):
    ...
    console.print("[bold cyan]📌 BỘ LỆNH ĐIỀU HÀNH HEO-AGENT TOÀN HỆ THỐNG:[/bold cyan]")
    console.print("[bold green]==============================================================================[/bold green]")
    console.print("\n [bold yellow]🚀 1. CÁC LỆNH KHỞI ĐỘNG HỆ THỐNG:[/bold yellow]")
    console.print("   👉 [bold cyan]heo-agent[/bold cyan]              : Khởi chạy hệ thống & tự động mở Web Dashboard")
    console.print("   👉 [bold cyan]heo-agent --bg[/bold cyan]         : Khởi chạy chế độ nền (Daemon 24/7 trực chiến ngầm)")
    console.print("   👉 [bold cyan]heo-agent tui[/bold cyan]          : Mở lại Trình cấu hình Terminal này")
    console.print("   👉 [bold cyan]heo-agent web[/bold cyan]          : Khởi chạy riêng và mở Web Console (cổng 5066)")
    console.print("\n [bold yellow]⚙️ 2. CÁC LỆNH THÔNG DỤNG HÀNG NGÀY:[/bold yellow]")
    console.print("   👉 [bold cyan]heo-agent status[/bold cyan]       : Giám sát trạng thái hoạt động (Zalo, Quota AI, Bot)")
    console.print("   👉 [bold cyan]heo-agent doctor[/bold cyan]       : Chẩn đoán toàn diện sức khỏe hệ thống (10 tiêu chuẩn)")
    console.print("   👉 [bold cyan]heo-agent doctor --fix[/bold cyan] : Tự động sửa chữa & phục hồi khi gặp sự cố (1-Click)")
    console.print("   👉 [bold cyan]heo-agent logs[/bold cyan]         : Xem luồng nhật ký hoạt động thời gian thực")
    console.print("   👉 [bold cyan]heo-agent model[/bold cyan]        : Xem và đổi mô hình AI (flash, pro, sonnet, opus)")
    console.print("   👉 [bold cyan]heo-agent effort[/bold cyan]       : Điều chỉnh mức suy luận logic (low, medium, high)")
    console.print("   👉 [bold cyan]heo-agent login-zalo[/bold cyan]   : Quét lại mã QR đăng nhập tài khoản Zalo")
    console.print("   👉 [bold cyan]heo-agent login-google[/bold cyan] : Xác thực / Đồng bộ Google AGY Core Agent")
    console.print("   👉 [bold cyan]heo-agent restart[/bold cyan]      : Khởi động lại toàn bộ dịch vụ Heo-Agent")
    console.print("   👉 [bold cyan]heo-agent stop[/bold cyan]         : Dừng toàn bộ hệ thống an toàn")
    console.print("   👉 [bold cyan]heo-agent uninstall[/bold cyan]    : Dọn dẹp sạch sẽ & khôi phục cài đặt gốc (Factory Reset)")
